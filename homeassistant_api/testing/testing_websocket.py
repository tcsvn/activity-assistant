...
import unittest
import asyncio
import homeassistant_api.websocket as ws
...
class TestHassWebsocketClassPerson(unittest.TestCase):
    ...
    @async_test
    async def test_person_list_stress(self):
        await self.ws.connect()
        for i in range(0, 100):
            print('id: ', self.ws._id_counter)
            test = await self.ws.list_person()
            print(test)
    ...
    @async_test
    async def test_person2person_id(self):
        await self.ws.connect()
        USER_ID = 15
        person_id = await self.ws.user_id2person_id(
            USER_ID)
        print(person_id)
    ...
    async def test_delete_person(self):
        await self.ws.connect()
        test = await self.ws.delete_person(5)
